...
这份手册是一份面向日常使用的中文操作指南。你可以把它当作一份“照着点”的说明：先进入工作台，再放入资料、发起任务、查看结果，最后根据需要配置模型、技能和更新。

> 说明：不同安装包或部署环境的按钮位置可能略有差异，但核心流程一致。
...
如果你拿到的是桌面应用或管理员提供的本机应用入口：

1. 打开 `InternAgents` 应用。
2. 等待本地服务启动完成。
3. 应用会自动进入工作台。

如果 macOS 提示无法验证开发者，可以在“应用程序”中按住 `Control` 点 `InternAgents`，再选择“打开”。

## 快速导览

打开工作台后，可以通过快速导览了解当前项目、工作区、聊天区、会话列表、配置和更新。

![快速导览](assets/user-manual/01-workbench-tour.png)
...
- `服务器地址`：InternAgents 服务地址，例如 `http://127.0.0.1:2024`。
- `助手 ID`：服务中的助手标识，例如 `agent`。
...
先看页面提示。常见原因是当前安装目录存在本地改动，或更新包暂时无法下载。

可以先重启应用，再进入 `关于与更新` 重试；如果仍然失败，请把页面提示发给管理员处理。
